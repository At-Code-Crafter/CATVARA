      ...
      <div class="d-flex align-items-center">
        ...
        <div>
          ...
          <div class="text-muted small">
            Update customer details for <span class="font-weight-bold">{{ $company->name }}</span>.
          </div>
        </div>
      </div>
    </div>
...
@section('content')
  ...
  <form action="{{ route('customers.update', [$company->uuid, $customer->id]) }}" class="ajax-form" method="POST">
    @csrf
    @method('PUT')
            ...
            <div class="card-body">
              <div class="d-flex flex-wrap" style="gap:10px;">
                <span class="ent-side-chip">
                  ...
                  <span id="chip_type">{{ old('type', $customer->type) }}</span>
                </span>
              ...
              <div class="form-group mb-3">
                ...
                <textarea name="notes"
                          ...
                          placeholder="Internal notes about this customer...">{{ old('notes', $customer->notes) }}</textarea>
                @error('notes')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
            ...
            <div class="card-body">
              <div class="d-flex">
                ...
                <div>
                  ...
                  <div class="text-muted small">
                    Keep <strong>Display Name</strong> business-friendly. Use <strong>Legal Name</strong> for invoices and compliance documents.
                  </div>
                </div>
              </div>
            </div>
  ...
  <script>
    $(function() {
      ...
      function syncCompanyCard() {
        ...
        // Hide card for INDIVIDUAL, show for COMPANY
        if (type === 'INDIVIDUAL') {
      ...
      function loadStates(countryUuid, selectedId = null) {
        ...
        $.get('/settings/countries/' + countryUuid + '/states', function(states) {
          states.forEach(function(state) {
            const selected = (selectedId && state.id == selectedId) ? true : false;
            $stateSelect.append(
              $('<option>', {
                value: state.id,
                text: state.name,
