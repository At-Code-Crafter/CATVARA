    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
      <div>
        <div class="flex items-center gap-3">
          <h2 class="text-3xl font-bold text-slate-800 tracking-tight">Edit Invoice #{{ $invoice->invoice_number }}</h2>
      ...
      <div class="flex items-center gap-3">
        <a href="{{ company_route('accounting.invoices.show', ['invoice' => $invoice->uuid]) }}" class="btn btn-white">
            ...
            <div class="p-6">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Issue Date</label>
                  <input type="date" name="issued_at" value="{{ $invoice->issued_at ? $invoice->issued_at->format('Y-m-d') : now()->format('Y-m-d') }}"
                ...
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Due Date</label>
                  <input type="date" name="due_date" value="{{ $invoice->due_date ? $invoice->due_date->format('Y-m-d') : '' }}"
                    class="w-full h-10 rounded-lg border-slate-200 text-sm font-semibold focus:border-brand-400 focus:ring-4 focus:ring-brand-400/10">
                </div>
                <div class="space-y-1.5">
                  <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Payment Term</label>
                  <select name="payment_term_id"
                    class="w-full h-10 rounded-lg border-slate-200 text-sm font-semibold focus:border-brand-400 focus:ring-4 focus:ring-brand-400/10">
                    <option value="">None</option>
                    @foreach ($paymentTerms as $term)
                      <option value="{{ $term->id }}" {{ $invoice->payment_term_id == $term->id ? 'selected' : '' }}>
          ...
          <div class="card border-none shadow-sm overflow-hidden bg-white">
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm" id="itemsTable">
                <thead class="bg-slate-50/50 text-slate-500 font-bold uppercase text-[10px] tracking-widest border-b border-slate-100">
                  <tr>
                ...
                <tbody class="divide-y divide-slate-50">
                  @foreach ($invoice->items as $index => $item)
                    <tr class="item-row hover:bg-slate-50/50 transition-colors" data-index="{{ $index }}">
                      <td class="px-4 py-3">
                        <input type="hidden" name="items[{{ $index }}][id]" value="{{ $item->id }}">
                        <div class="flex flex-col">
                          <span class="font-bold text-slate-800 text-xs">{{ $item->product_name }}</span>
                          @if ($item->variant_description)
                            <span class="text-[10px] text-slate-400">{{ $item->variant_description }}</span>
                          @endif
                        </div>
                      </td>
                      <td class="px-4 py-3">
                        <div class="flex items-center gap-1">
                          <button type="button" class="qty-minus flex-shrink-0 w-7 h-9 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-sm transition-all flex items-center justify-center" data-index="{{ $index }}">
                            <i class="fas fa-minus text-[10px]"></i>
                          </button>
                          <input type="number" name="items[{{ $index }}][quantity]" value="{{ (int) $item->quantity }}"
                            step="1" min="1"
                          ...
                          <button type="button" class="qty-plus flex-shrink-0 w-7 h-9 rounded-lg bg-slate-100 hover:bg-brand-100 text-slate-600 hover:text-brand-600 font-bold text-sm transition-all flex items-center justify-center" data-index="{{ $index }}">
                            <i class="fas fa-plus text-[10px]"></i>
                          </button>
                        </div>
                      </td>
                      <td class="px-4 py-3">
                        <input type="number" name="items[{{ $index }}][unit_price]" value="{{ (float) $item->unit_price }}"
                          step="0.01" min="0"
                      ...
                      <td class="px-4 py-3">
                        <input type="number" name="items[{{ $index }}][discount_percent]" value="{{ (float) $item->discount_percent }}"
                      ...
                      <td class="px-4 py-3">
                        <select name="items[{{ $index }}][tax_group_id]"
                          ...
                          @foreach ($taxGroups as $tg)
                            <option value="{{ $tg->id }}" data-rate="{{ $tg->activeRateSum() }}"
        ...
        <div class="space-y-6">
          ...
          <div class="card border-none shadow-sm overflow-hidden bg-white sticky top-6">
            ...
            <div class="p-6 space-y-5">
              <div class="space-y-1.5">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Global Discount (%)</label>
                <input type="number" name="global_discount_percent" id="globalDiscountPercent"
                  value="{{ (float) $invoice->global_discount_percent }}" step="0.01" min="0" max="100"
                  class="w-full h-10 rounded-lg border-slate-200 text-sm font-semibold focus:border-brand-400 focus:ring-4 focus:ring-brand-400/10">
              </div>
              <div class="space-y-1.5">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Shipping Total</label>
              ...
              <div class="space-y-3">
                ...
                <div class="pt-4 mt-4 border-t border-slate-800">
                  <div class="flex justify-between items-end">
                    <span class="text-[10px] font-black uppercase tracking-[0.2em] text-brand-400">Grand Total</span>
                    <span class="text-3xl font-black tracking-tight text-brand-500" id="displayGrandTotal">0.00</span>
                  </div>
                </div>
              </div>
            </div>
