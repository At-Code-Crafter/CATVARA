  ...
  <div class="w-full mx-auto animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-6">
      <div>
        ...
        <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Modules</h1>
        <p class="text-slate-500 font-medium mt-1">Manage system modules and feature groupings.</p>
      </div>
      <a href="{{ route('modules.create') }}" class="btn btn-primary shadow-lg shadow-brand-500/30">
    ...
    <div class="card bg-white border-slate-100 shadow-soft overflow-hidden">
      <div class="p-6">
        <table id="modulesTable" class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-100">
              <th class="text-left py-3 px-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Module</th>
              <th class="text-left py-3 px-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Slug</th>
  ...
  <script>
    $(document).ready(function() {
      $('#modulesTable').DataTable({
        ...
        columns: [
          { data: 'name_html', name: 'modules.name', orderable: true, searchable: true },
          { data: 'slug_html', name: 'modules.slug', orderable: true, searchable: true },
          { data: 'permissions_html', name: 'permissions_count', orderable: false, searchable: false },
          { data: 'status_html', name: 'modules.is_active', orderable: true, searchable: false },
        ...
        language: {
          ...
          emptyTable: '<div class="py-12 text-center"><div class="w-16 h-16 rounded-2xl bg-purple-50 flex items-center justify-center mx-auto mb-4"><i class="fas fa-cubes text-2xl text-purple-300"></i></div><p class="text-slate-500 font-medium">No modules yet</p><p class="text-slate-400 text-xs mt-1">Create your first module to get started</p></div>',
          zeroRecords: '<div class="py-8 text-center"><i class="fas fa-search text-slate-300 text-2xl mb-2"></i><p class="text-slate-500 font-medium">No matching modules found</p></div>'
        },
