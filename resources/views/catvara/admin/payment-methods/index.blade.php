  ...
  <div class="w-full mx-auto animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-6">
      <div>
        ...
        <p class="text-slate-500 font-medium mt-1">View all payment methods across all companies.</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      <div class="card p-6 bg-white border-slate-100 shadow-soft hover:shadow-md transition-all">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-2xl font-bold text-slate-800">{{ \App\Models\Accounting\PaymentMethod::count() }}</p>
            <p class="text-xs text-slate-400 font-medium uppercase tracking-wide">Total Methods</p>
          </div>
        </div>
      </div>
      <div class="card p-6 bg-white border-slate-100 shadow-soft hover:shadow-md transition-all">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-2xl font-bold text-slate-800">{{ \App\Models\Accounting\PaymentMethod::where('is_active', true)->count() }}</p>
      ...
      <div class="card p-6 bg-white border-slate-100 shadow-soft hover:shadow-md transition-all">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-2xl font-bold text-slate-800">{{ \App\Models\Accounting\PaymentMethod::distinct('company_id')->count('company_id') }}</p>
      ...
      <div class="card p-6 bg-white border-slate-100 shadow-soft hover:shadow-md transition-all">
        <div class="flex items-center gap-4">
          <div class="w-12 h-12 rounded-xl bg-amber-50 flex items-center justify-center shrink-0">
            <i class="fas fa-exchange-alt text-amber-500 text-lg"></i>
          </div>
          <div>
            <p class="text-2xl font-bold text-slate-800">{{ \App\Models\Accounting\PaymentMethod::where('allow_refund', true)->count() }}</p>
            <p class="text-xs text-slate-400 font-medium uppercase tracking-wide">Refund Enabled</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card bg-white border-slate-100 shadow-soft overflow-hidden">
      <div class="p-6">
        <table id="paymentMethodsTable" class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-100">
              <th class="text-left py-3 px-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Method</th>
              ...
              <th class="text-left py-3 px-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Features</th>
  ...
  <script>
    $(document).ready(function() {
      $('#paymentMethodsTable').DataTable({
        ...
        columns: [
          { data: 'name_html', name: 'payment_methods.name', orderable: true, searchable: true },
          { data: 'company_html', name: 'companies.name', orderable: true, searchable: true },
          { data: 'type_html', name: 'payment_methods.type', orderable: true, searchable: false },
          { data: 'features_html', name: 'features', orderable: false, searchable: false },
          { data: 'status_html', name: 'payment_methods.is_active', orderable: true, searchable: false }
        ],
        order: [[1, 'asc'], [0, 'asc']],
        pageLength: 25,
        language: {
          ...
          infoEmpty: "No payment methods found",
          infoFiltered: "(filtered from _MAX_ total)",
          emptyTable: '<div class="py-12 text-center"><div class="w-16 h-16 rounded-2xl bg-blue-50 flex items-center justify-center mx-auto mb-4"><i class="fas fa-credit-card text-2xl text-blue-300"></i></div><p class="text-slate-500 font-medium">No payment methods yet</p></div>',
          zeroRecords: '<div class="py-8 text-center"><i class="fas fa-search text-slate-300 text-2xl mb-2"></i><p class="text-slate-500 font-medium">No matching methods found</p></div>'
        },
