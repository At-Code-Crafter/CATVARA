    ...
    <div class="space-y-8 animate-fade-in">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <span
                        class="badge {{ ($order->status->code ?? '') === 'CONFIRMED' ? 'badge-success' : 'badge-warning' }}">
            ...
            <div class="flex items-center gap-3">
                ...
                @if ($canDeliver)
                    <a href="{{ company_route('sales-orders.delivery-note.create', ['sales_order' => $order->uuid]) }}"
                        target="_blank" class="btn btn-primary bg-indigo-600 hover:bg-indigo-700 text-white">
                ...
                <a href="{{ company_route('sales-orders.box-labels', ['sales_order' => $order->uuid]) }}"
                    class="btn btn-white">
                    <i class="fas fa-box mr-2"></i> Box Labels
                </a>
                <a href="{{ company_route('sales-orders.print-proforma', ['sales_order' => $order->uuid]) }}"
        ...
        <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-soft overflow-hidden relative">
            <div class="flex justify-between items-end mb-4">
                <div>
                    ...
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                        ...
                        &nbsp;•&nbsp; Completion: <span
                            class="text-indigo-600 font-black">{{ $stats['percentage'] }}%</span>
                    </p>
                </div>
                <div class="flex gap-4">
                    <div class="text-right">
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Fully
                            Shipped</span>
                        <span class="text-sm font-black text-slate-800">{{ $stats['fully'] }} /
            ...
            <div class="h-3 bg-slate-50 rounded-full overflow-hidden flex p-0.5 border border-slate-100 shadow-inner">
                <div class="h-full bg-linear-to-r from-indigo-500 to-brand-500 rounded-full shadow-lg shadow-indigo-200/50 transition-all duration-[1500ms]"
        ...
        @if ($companyBank)
            <div class="card p-6 border-slate-100 shadow-soft relative overflow-hidden">
                ...
                <div class="flex items-center gap-3 mb-6">
                    <div
                        class="h-10 w-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-500 shadow-sm">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    @if ($companyBank->iban)
                        <div class="bg-indigo-50 rounded-xl p-4 md:col-span-2">
                            ...
                            <p class="font-bold text-indigo-700 font-mono text-sm tracking-wide">{{ $companyBank->iban }}
                            </p>
                        </div>
                    @endif
                            ...
                            <tr>
                                ...
                                <td class="text-center min-w-[120px]">
                                    @php
                                        $percent =
                                            $item->quantity > 0
                                                ? ($item->fulfilled_quantity / $item->quantity) * 100
                                                : 0;
                                        $barColor =
                                            $percent >= 100
                                                ? 'bg-emerald-500'
                                    ...
                                    <div class="flex flex-col items-center gap-1">
                                        <div
                                            class="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden max-w-[100px] mx-auto">
                                ...
                                <td class="text-right font-medium text-slate-600">
                                ...
                                <td class="text-right px-8! font-bold text-slate-900">
            ...
            <div class="lg:w-1/2">
                @if ($order->notes)
                    <div class="card p-6 bg-slate-50/50 border-none shadow-none">
                        <h4
                            class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 border-b border-white/50 pb-2">
                            Internal Notes</h4>
                ...
                <div class="card p-6 border-slate-100 shadow-soft space-y-4">
                    ...
                    @if ($order->discount_total > 0)
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <span
                                class="font-bold text-emerald-500">-{{ number_format((float) $order->discount_total, 2) }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-400 font-medium">Shipping & Additional</span>
                        <span
                            class="font-bold text-slate-700">{{ number_format((float) $order->shipping_total, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                    ...
                    <div class="pt-4 border-t border-slate-50 flex justify-between items-center">
                        ...
                        <span
                            class="text-2xl font-black text-brand-400">{{ number_format((float) $order->grand_total, 2) }}
            ...
            <div class="card p-6 border-slate-100 shadow-soft">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center gap-4">
                        <div
                            class="h-10 w-10 rounded-xl bg-white shadow-xs flex items-center justify-center text-indigo-500">
                            <i class="fas fa-boxes text-sm"></i>
                        </div>
                        <div>
                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Total
                                Line
                                Items</span>
                    ...
                    <div class="p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex items-center gap-4">
                        <div
                            class="h-10 w-10 rounded-xl bg-white shadow-xs flex items-center justify-center text-emerald-500">
                            <i class="fas fa-check-double text-sm"></i>
                        </div>
                        <div>
                            <span
                                class="text-[9px] font-black text-indigo-400 uppercase tracking-widest block mb-0.5">Fully
                                Shipped</span>
                    ...
                    <div class="p-4 rounded-2xl bg-orange-50/50 border border-orange-100 flex items-center gap-4">
                        <div
                            class="h-10 w-10 rounded-xl bg-white shadow-xs flex items-center justify-center text-orange-400">
                            <i class="fas fa-hourglass-half text-sm"></i>
                        </div>
                        <div>
                            <span
                                class="text-[9px] font-black text-orange-400 uppercase tracking-widest block mb-0.5">Pending
                                Shipment</span>
                    ...
                    <div class="p-4 rounded-2xl bg-emerald-50/50 border border-emerald-100 flex items-center gap-4">
                        <div
                            class="h-10 w-10 rounded-xl bg-white shadow-xs flex items-center justify-center text-brand-500">
                            <i class="fas fa-weight-hanging text-sm"></i>
                        </div>
                        <div>
                            <span
                                class="text-[9px] font-black text-emerald-400 uppercase tracking-widest block mb-0.5">Weight
                                / Value
                                Ratio</span>
    ...
    <script>
        function confirmDeleteNote(dnUuid) {
            Swal.fire({
                ...
                preConfirm: () => {
                    let url =
                        "{{ company_route('sales-orders.delivery-note.delete', ['delivery_note' => ':uuid']) }}";
                    ...
                    return $.ajax({
                        ...
                        Swal.showValidationMessage(
                            `Error: ${xhr.responseJSON?.message || 'Deletion failed'}`);
                    });
                }
            }).then((result) => {
        ...
        function markAsDelivered(dnUuid) {
            Swal.fire({
                ...
                preConfirm: () => {
                    let url =
                        "{{ company_route('sales-orders.delivery-note.delivered', ['delivery_note' => ':uuid']) }}";
                    url = url.replace(':uuid', dnUuid);
                    ...
                    return $.ajax({
                        ...
                        Swal.showValidationMessage(
                            `Error: ${xhr.responseJSON?.message || 'Update failed'}`);
                    });
                }
            }).then((result) => {
                if (result.isConfirmed && result.value.ok) {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.post(
                        ...
                        Swal.showValidationMessage(
                            `Error: ${xhr.responseJSON?.message || 'Fulfillment failed'}`);
                    });
                }
            }).then((result) => {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.post(
                        ...
                        Swal.showValidationMessage(
                            `Error: ${xhr.responseJSON?.message || 'Generation failed'}`);
                    });
                }
            }).then((result) => {
                if (result.isConfirmed && result.value.success) {
