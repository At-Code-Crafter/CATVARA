...
@section('content')
  ...
  <div class="w-full px-8 pb-20 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
      <div>
        ...
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Add Items for {{ $quote->quote_number }}</h1>
      </div>
          ...
          <div class="p-4 border-b border-slate-50 bg-slate-50/50">
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-2">
                <i class="fas fa-file-alt text-brand-500"></i> Quote Draft
              </h3>
            ...
            <div class="space-y-3">
              <div class="grid grid-cols-1 gap-2">
                <div class="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    <label class="text-[10px] font-bold text-slate-500 uppercase">Valid Until</label>
                    <input id="validUntilDate" type="text"
                      class="w-full h-8 rounded border-slate-200 text-xs font-bold bg-white text-slate-700 text-center cursor-pointer"
  ...
  {{-- Hidden Input for Quote ID --}}
  <input type="hidden" id="quoteUuid" value="{{ $quote->uuid }}">
  <input type="hidden" id="quoteCurrency" value="{{ $quote->currency->code ?? 'AED' }}">
...
@push('scripts')
  <script>
    ...
    const quoteUpdateUrl = "{{ company_route('quotes.update', ['quote' => $quote->uuid]) }}";
    ...
    $(document).ready(function() {
      ...
      // Init flatpickr for valid until date
      flatpickr("#validUntilDate", {
        dateFormat: "Y-m-d",
        defaultDate: initialState?.valid_until || null,
        minDate: "today"
      });
                ...
                <div class="card bg-white border border-slate-100 shadow-sm hover:shadow-md hover:border-brand-200 transition-all duration-300 group cursor-pointer animate-entry"
                    ...
                    <div class="p-3">
                        ...
                        <div class="flex items-center justify-between mt-auto">
                            <span class="text-sm font-black text-slate-800">${price} <span class="text-[9px] text-slate-400 font-bold uppercase ml-0.5">{{ $quote->currency->code ?? 'AED' }}</span></span>
        ...
        const item = `
                <label class="relative flex cursor-pointer rounded-lg border border-slate-200 bg-white p-3 hover:border-brand-300 hover:bg-brand-50/10 transition-all focus-within:ring-2 focus-within:ring-brand-500 focus-within:ring-offset-2">
                    ...
                    <span class="flex flex-col text-right">
                       ...
                       <span class="text-[9px] text-slate-400 font-bold uppercase">{{ $quote->currency->code ?? 'AED' }}</span>
                    </span>
    ...
    function saveQuote(action) {
      const btn = action === 'next_step' ? $('#nextStepBtn') : $('#saveDraftBtn');
      ...
      $.ajax({
        ...
        success: function(response) {
          if (action === 'next_step') {
            window.location.href = "{{ company_route('quotes.show', ['quote' => $quote->id]) }}";
          } else {
