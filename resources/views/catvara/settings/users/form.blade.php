...
@section('content')
    <div class="w-full mx-auto pb-24 animate-fade-in">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span
                        class="text-[10px] font-black px-2 py-0.5 rounded bg-slate-100 text-slate-500 uppercase tracking-widest">
                        Team Management
                    </span>
                </div>
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('settings.users.index', ['company' => $company->uuid]) }}"
                    class="btn btn-white shadow-soft">
                    Cancel
                </a>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="card p-8 bg-white border-slate-100 shadow-soft relative overflow-hidden">
                        ...
                        <div class="flex items-center gap-4 mb-8">
                            <div
                                class="h-10 w-10 rounded-xl bg-brand-50 text-brand-500 flex items-center justify-center shadow-sm">
                    ...
                    <div class="card p-8 bg-white border-slate-100 shadow-soft relative overflow-hidden">
                        ...
                        <div class="flex items-center gap-4 mb-8">
                            <div
                                class="h-10 w-10 rounded-xl bg-purple-50 text-purple-500 flex items-center justify-center shadow-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach ($roles as $role)
                                <label
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-slate-700 group-hover:text-slate-900">
                                            {{ $role->name }}</p>
                                        @if ($role->description)
                    ...
                    {{-- Brand Restrictions Card --}}
                    <div class="card p-8 bg-white border-slate-100 shadow-soft relative overflow-hidden">
                        <div class="absolute top-0 left-0 w-1 h-full bg-amber-400"></div>
                        <div class="flex items-center gap-4 mb-4">
                            <div
                                class="h-10 w-10 rounded-xl bg-amber-50 text-amber-500 flex items-center justify-center shadow-sm">
                                <i class="fas fa-tags"></i>
                            </div>
                            <div>
                                <h3 class="text-lg font-black text-slate-800 tracking-tight">Brand Restrictions</h3>
                                <p class="text-xs text-slate-400 font-bold uppercase tracking-widest">Product Visibility</p>
                            </div>
                        </div>
                        <p class="text-xs text-slate-500 font-medium mb-6">
                            Select which brands this user can see. If no brands are selected, the user will have access to
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            @foreach ($brands ?? [] as $brand)
                                <label
                                    class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:border-amber-200 hover:bg-amber-50/30 transition-all cursor-pointer group has-[:checked]:bg-amber-50 has-[:checked]:border-amber-200">
                                    <input type="checkbox" name="brand_ids[]" value="{{ $brand->id }}"
                                        ...
                                        {{ in_array($brand->id, old('brand_ids', $userBrandIds ?? [])) ? 'checked' : '' }}>
                                    <div class="flex items-center gap-2 min-w-0">
                                        @if ($brand->logo)
                                            <img src="{{ asset('storage/' . $brand->logo) }}"
                                                class="h-6 w-6 rounded object-contain" alt="">
                                        @endif
                                        <span
                                            class="text-sm font-bold text-slate-700 group-hover:text-slate-900 truncate">{{ $brand->name }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>

                        @if (empty($brands) || (isset($brands) && $brands->isEmpty()))
                            <p class="text-xs text-slate-400 italic">No brands found for this company.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-sky-50/50 rounded-2xl p-6 border border-sky-100">
                        <h4
                            class="text-[11px] font-black text-sky-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                    ...
                    @if (isset($user))
                        <div class="card p-6 bg-white border-rose-100 shadow-soft">
                            <h3
                                class="text-sm font-black text-rose-500 uppercase tracking-widest mb-4 border-b border-rose-50 pb-4">
                                Danger Zone
                            </h3>
        ...
        @if (isset($user))
            <form id="delete-form" action="{{ route('settings.users.destroy', [$company->uuid, $user->id]) }}"
