  ...
  <div class="space-y-8 animate-fade-in pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
      <div>
        ...
        <p class="text-slate-400 text-sm font-bold flex items-center gap-2">
          Configure payment deadlines and conditions
        </p>
      </div>
      <div>
        <a href="{{ company_route('settings.payment-terms.create') }}"
          class="btn btn-primary shadow-lg shadow-brand-500/30">
          <i class="fas fa-plus-circle mr-2"></i> Create New Term
        </a>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="card p-6 border-b-4 border-b-blue-400 hover:-translate-y-1 transition-transform duration-300">
        ...
        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">All Terms</p>
        <p class="text-2xl font-black text-slate-800" id="statTotalTerms">0</p>
      </div>
      ...
      <div class="card p-6 border-b-4 border-b-emerald-400 hover:-translate-y-1 transition-transform duration-300">
        ...
        <p class="text-2xl font-black text-slate-800" id="statActiveTerms">0</p>
      </div>
      ...
      <div class="card p-6 border-b-4 border-b-slate-400 hover:-translate-y-1 transition-transform duration-300">
        ...
        <p class="text-2xl font-black text-slate-800" id="statInactiveTerms">0</p>
      </div>
    </div>
...
@push('scripts')
  <script>
    $(function() {
      function loadStats() {
        $.get('{{ company_route('settings.payment-terms.index') }}', {
          stats: true
        }, function(data) {
          const total = data.data ? data.data.length : 0;
          const active = data.data ? data.data.filter(t => t.is_active).length : 0;
          const inactive = total - active;

          $('#statTotalTerms').text(total);
          $('#statActiveTerms').text(active);
          $('#statInactiveTerms').text(inactive);
        });
      }
      ...
      const table = $('.data-table').DataTable({
        ...
        ajax: {
          url: '{{ company_route('settings.payment-terms.index') }}'
        },
        columns: [{
            data: 'code',
          ...
          },
          {
            ...
            render: (data) => {
              if (data === 0) {
                return '<span class="px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 text-xs font-black uppercase tracking-wider border border-amber-100">Immediate</span>';
              }
              return `<span class="font-bold text-slate-600">${data} days</span>`;
            }
          },
          {
            data: 'is_active',
