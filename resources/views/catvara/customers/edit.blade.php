            ...
            <div class="space-y-6">
              <div class="space-y-1.5">
                ...
                <textarea name="address_line_1" rows="3" class="w-full py-2.5 font-semibold bg-slate-50 border-slate-200"
                  placeholder="Street name, building, unit number...">{{ old('address_line_1', $customer->billingAddress?->address_line_1) }}</textarea>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <select name="country_id" id="country_id" class="w-full pt-1">
                    ...
                    @foreach ($countries as $country)
                      <option value="{{ $country->id }}" data-uuid="{{ $country->uuid }}"
                        {{ old('country_id', $customer->billingAddress?->country_id) == $country->id ? 'selected' : '' }}>
                        {{ $country->name }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="space-y-1.5">
                  ...
                  <select name="state_id" id="state_id" class="w-full pt-1">
                    ...
                    @foreach ($states as $st)
                      <option value="{{ $st->id }}"
                        {{ old('state_id', $customer->billingAddress?->state_id) == $st->id ? 'selected' : '' }}>
                        {{ $st->name }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="input-icon-group">
                    ...
                    <input type="text" name="city" value="{{ old('city', $customer->billingAddress?->city) }}"
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="input-icon-group">
                    ...
                    <input type="text" name="zip_code" value="{{ old('zip_code', $customer->billingAddress?->zip_code) }}"
                    ...
                    @foreach ($groupedTimezones as $region => $tzList)
                      <optgroup label="{{ $region }}">
                        @foreach ($tzList as $tz)
                          <option value="{{ $tz }}"
                            {{ old('timezone', $customer->timezone) == $tz ? 'selected' : '' }}>
                            {{ str_replace('_', ' ', $tz) }}
                          </option>
                        @endforeach
                      </optgroup>
              ...
              {{-- Shipping Address --}}
              @php $shipSame = old('shipping_same_as_billing', $customer->shippingAddress ? '0' : '1'); @endphp
                ...
                <div id="shipping_fields" class="space-y-6">
                  <div class="space-y-1.5">
                    <textarea name="shipping_address_line_1" rows="3"
                      ...
                      placeholder="Street name, building, unit number...">{{ old('shipping_address_line_1', $customer->shippingAddress?->address_line_1) }}</textarea>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                      ...
                      <select name="shipping_country_id" id="shipping_country_id" class="w-full pt-1">
                        ...
                        @foreach ($countries as $country)
                          <option value="{{ $country->id }}" data-uuid="{{ $country->uuid }}"
                            {{ old('shipping_country_id', $customer->shippingAddress?->country_id) == $country->id ? 'selected' : '' }}>
                            {{ $country->name }}
                          </option>
                        @endforeach
                      </select>
                    ...
                    <div class="space-y-1.5">
                      <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1">State /
                        Region</label>
                      <select name="shipping_state_id" id="shipping_state_id" class="w-full pt-1">
                        <option value="">Select state...</option>
                        @foreach ($shippingStates as $st)
                          <option value="{{ $st->id }}"
                            {{ old('shipping_state_id', $customer->shippingAddress?->state_id) == $st->id ? 'selected' : '' }}>
                            {{ $st->name }}
                          </option>
                        @endforeach
                      </select>
                    ...
                    <div class="space-y-1.5">
                      ...
                      <div class="input-icon-group">
                        ...
                        <input type="text" name="shipping_city"
                          value="{{ old('shipping_city', $customer->shippingAddress?->city) }}"
                          class="w-full py-2.5 font-semibold placeholder:font-normal" placeholder="e.g. London">
                      </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                      ...
                      <div class="input-icon-group">
                        ...
                        <input type="text" name="shipping_zip_code"
                          value="{{ old('shipping_zip_code', $customer->shippingAddress?->zip_code) }}"
                          class="w-full py-2.5 font-semibold placeholder:font-normal" placeholder="e.g. SW1E 5JL">
                      </div>
                    </div>
                  </div>
              ...
              <div class="space-y-1.5">
                ...
                <select name="tax_group_id" class="w-full pt-1">
                  ...
                  @foreach ($taxGroups as $tg)
                    <option value="{{ $tg->id }}"
                      {{ old('tax_group_id', $customer->tax_group_id) == $tg->id ? 'selected' : '' }}>
                      {{ $tg->name }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="pt-2 border-t border-slate-50 space-y-3">
                ...
                <div id="tax_exempt_reason_wrap" class="{{ old('is_tax_exempt', '0') == '1' ? '' : 'hidden' }}">
                  ...
                  <div class="input-icon-group">
                    ...
                    <input type="text" name="tax_exempt_reason" value="{{ old('tax_exempt_reason', $customer->tax_exempt_reason) }}"
