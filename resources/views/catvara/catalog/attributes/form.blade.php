        ...
        <div class="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden">
          ...
          <div class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="name" class="block text-sm font-semibold text-slate-700 mb-2">Display Name <span
              ...
              <div>
                <label for="code" class="block text-sm font-semibold text-slate-700 mb-2">Attribute Code <span
                    class="text-red-500">*</span></label>
                ...
                @error('code')
                  <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
                <p class="mt-2 text-xs text-slate-500">Lowercase, no spaces. Used for filters and API.</p>
              </div>
            </div>
            ...
            <div>
              <div class="mb-4">
                <h4 class="text-sm font-bold text-slate-800">Attribute Values</h4>
                <p class="text-xs text-slate-500 mt-1">Define selectable options for this attribute.</p>
              </div>
              ...
              @if (isset($attribute))
                ...
                <div>
                  <label for="new_values" class="block text-sm font-semibold text-slate-700 mb-2">Add New Values</label>
              ...
              @else
                <div>
                  <label for="values" class="block text-sm font-semibold text-slate-700 mb-2">Initial Values <span
                      class="text-red-500">*</span></label>
                  <textarea name="values" id="values" rows="3"
                    class="w-full rounded-xl border-slate-200 focus:border-brand-500 focus:ring-brand-500 shadow-sm placeholder-slate-400 text-sm py-2.5 transition-all"
                    placeholder="Enter values separated by comma (e.g. Red, Blue, Green)" required>{{ old('values') }}</textarea>
                  @error('values')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                  @enderror
                  <p class="mt-2 text-xs text-slate-500">Separate multiple values with commas.</p>
                </div>
              @endif
            </div>
          </div>
        ...
        <div class="bg-white rounded-2xl shadow-soft border border-slate-100 p-6 sticky top-6">
          ...
          <div class="mt-8 pt-6 border-t border-slate-50">
            <p class="text-xs text-slate-500 leading-relaxed">
              ...
              Attributes help customers filter products and allow you to create product variants (like Size and Color
              combinations).
            </p>
          </div>
        </div>
