      ...
      <div class="lg:col-span-8 space-y-8">
        <div class="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden">
          <div class="p-6 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
            <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider">Category Details</h3>
          ...
          <div class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 gap-6">
              ...
              <div>
                ...
                <p class="mt-2 text-xs text-slate-500">Nest this category under another one to create a hierarchy.</p>
                @error('parent_id')
                  <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
              </div>
            ...
            <div>
              <div class="mb-4">
                <h4 class="text-sm font-bold text-slate-800">Related Attributes</h4>
                <p class="text-xs text-slate-500 mt-1">Products in this category will inherit these attributes for variant
                  creation.</p>
              </div>
              ...
              <select name="attributes[]" id="attributes" class="select2 w-full" multiple
                ...
                @foreach ($attributes as $attr)
                  <option value="{{ $attr->id }}"
                    {{ collect(old('attributes', isset($category) ? $category->attributes->pluck('id') : []))->contains($attr->id) ? 'selected' : '' }}>
                    {{ $attr->name }}
                  </option>
                @endforeach
              </select>
